...
1. 强调在协同知识图谱中显式建模高阶关系的重要性，以利用 **物品侧** 信息提供更优的推荐
2. 提出了一种新方法 **KGAT**，在图神经网络框架下以显式且端到端的方式实现高阶关系建模
3. 在三个公开基准数据集上进行了大量实验，验证了 KGAT 的有效性及其在理解高阶关系重要性方面的可解释性
...
1. 协同知识图谱（CKG）：将用户-物品交互与知识图谱统一建模为协同知识图谱，在统一图结构上学习用户、物品及实体的表示，实现协同信号与知识信息的融合
2. 递归嵌入传播：根据节点邻居的嵌入更新其自身嵌入，通过递归方式进行嵌入传播，捕捉高阶连接性
3. 基于注意力的聚合机制：使用注意力机制学习不同邻居在信息传播过程中的贡献程度，从而增强有用知识的传播并抑制噪声信息
